...
function getInvestimentoInfo(t) {
    const m = {
        cdb: { icon: '🏦', label: 'CDB' },
        tesouro: { icon: '🇧🇷', label: 'Tesouro Direto' },
        fundo: { icon: '📊', label: 'Fundo' },
        acoes: { icon: '📈', label: 'Ações' },
        fii: { icon: '🏢', label: 'FII' },
        cripto: { icon: '₿', label: 'Cripto' },
        poupanca: { icon: '🐷', label: 'Poupança' },
        previdencia: { icon: '🔮', label: 'Previdência' },
...
function navigateTo(screenId) {
    ...
    if (activeNav) {
        document.querySelectorAll(`.nav-item[data-nav="${activeNav}"]`).forEach(n => n.classList.add('active'));
    }
...
function updateSaldoInicialStatus(conta, investido) {
    ...
    if (conta > 0 || investido > 0) {
        el.innerHTML = `<div style="padding:8px;background:var(--bg-card-alt);border-radius:8px;font-size:13px;">
...
async function updateDashboard() {
    ...
    if (ultimas.length === 0) {
        ...
        listEl.querySelectorAll('.mov-item').forEach(i => {
            i.addEventListener('click', () => openDetalhe(i.dataset.id));
        });
    }
...
function renderMovItem(r) {
    ...
    return `<div class="mov-item" data-id="${r.id}">
        <div class="mov-icon ${r.tipo}">${icon}</div>
        <div class="mov-info"><div class="mov-desc">${desc}${futuroTag}</div><div class="mov-meta">${meta}</div></div>
        <div class="mov-valor ${r.tipo}">${prefix}${formatMoney(r.valor)}</div>
    </div>`;
}
...
function updateFormVisibility() {
    const isG = tipoRegistro === 'gasto';
    const isI = tipoRegistro === 'investimento';
    const isR = tipoRegistro === 'resgate';
...
async function salvarRegistro() {
    ...
    if (tipoRegistro === 'gasto') {
        reg.categoria = document.getElementById('select-categoria').value;
        reg.metodo = metodoSelecionado;
    } else if (tipoRegistro === 'receita') {
        reg.metodo = metodoSelecionado;
    } else if (tipoRegistro === 'investimento' || tipoRegistro === 'resgate') {
        reg.tipoInvestimento = document.getElementById('select-tipo-investimento').value;
    }
...
function initHistorico() {
    ...
    document.getElementById('filtro-mes-prev').addEventListener('click', () => {
        filtroTodosMeses = false;
        filtroMes--;
        if (filtroMes < 0) { filtroMes = 11; filtroAno--; }
        updateHistorico();
    });

    document.getElementById('filtro-mes-next').addEventListener('click', () => {
        filtroTodosMeses = false;
        filtroMes++;
        if (filtroMes > 11) { filtroMes = 0; filtroAno++; }
        updateHistorico();
    });

    document.getElementById('filtro-todos-meses').addEventListener('click', () => {
        filtroTodosMeses = !filtroTodosMeses;
        updateHistorico();
    });
}
...
async function updateHistorico() {
    ...
    if (!filtroTodosMeses) {
        filtered = filtered.filter(r => {
            const [y, m] = r.data.split('-').map(Number);
            return y === filtroAno && (m - 1) === filtroMes;
        });
    }
    ...
    let soma = 0;
    filtered.forEach(r => {
        if (r.tipo === 'receita' || r.tipo === 'resgate') soma += r.valor;
        else soma -= r.valor;
    });
...
async function applyThemeFromSaved() {
    const p = await loadThemePreference();
    if (p.auto !== false) applyTheme(window.matchMedia('(prefers-color-scheme: dark)').matches);
    else applyTheme(p.dark === true);
}

async function loadThemePreference() {
    try {
        const c = await dbGet('config', 'theme');
        if (c && c.valor) return c.valor;
    } catch (e) { }
    return { auto: true, dark: false };
}

async function saveThemePreference(p) {
    try { await dbPut('config', { chave: 'theme', valor: p }); } catch (e) { }
}
...
async function initMetas() {
    ...
    document.getElementById('modal-meta-overlay').addEventListener('click', (e) => {
...
function closeMetaModal() {
    document.getElementById('modal-meta-overlay').classList.remove('open');
}
...
async function updateMetas() {
    ...
    const orcamento = orcConfig ? orcConfig.valor : 0;
    if (orcamento > 0) {
        document.getElementById('input-orcamento-geral').value = Number(orcamento).toLocaleString('pt-BR', { minimumFractionDigits: 2 });
    }
    ...
    const metasConfig = await dbGet('config', 'metas-categorias');
    const metas = metasConfig ? metasConfig.valor : {};
    ...
    document.getElementById('metas-categorias-list').innerHTML = cats.map(cat => {
        ...
        return `<div class="meta-cat-item" data-cat="${cat}">
            ...
            <div class="meta-cat-info"><div class="meta-cat-name">${info.label}</div><div class="meta-cat-status">${hm ? `${formatMoney(gasto)} / ${formatMoney(meta)}` : 'Sem meta definida'}</div></div>
            ${hm ? `<span class="meta-cat-valor">${Math.round((gasto / meta) * 100)}%</span>` : ''}
            <span class="meta-cat-arrow">→</span>
        </div>`;
    }).join('');

    document.querySelectorAll('.meta-cat-item').forEach(i => {
        i.addEventListener('click', () => openMetaModal(i.dataset.cat));
    });
...
function renderProgress(label, gasto, meta, pct, level) {
    ...
    return `<div class="progress-container">
        <div class="progress-header"><span class="progress-label">${label}</span><span class="progress-values"><strong>${formatMoney(gasto)}</strong> / ${formatMoney(meta)}</span></div>
        <div class="progress-bar"><div class="progress-fill ${level}" style="width:${Math.min(pct, 100)}%"></div></div>
        <div class="progress-percent ${level}">${pct.toFixed(0)}%</div>
        ${alert}
    </div>`;
}

async function openMetaModal(cat) {
    currentMetaCategoria = cat;
    const info = getCategoriaInfo(cat);
    ...
    const mc = await dbGet('config', 'metas-categorias');
    const metas = mc ? mc.valor : {};
    const v = metas[cat] || 0;
...
async function updateOrcamentoPreview(totalGastosMes) {
    ...
    const metasConfig = await dbGet('config', 'metas-categorias');
    const metas = metasConfig ? metasConfig.valor : {};
    const hasMetas = Object.values(metas).some(v => v > 0);
    ...
    if (orcamento > 0) {
        ...
        document.getElementById('orcamento-geral').innerHTML = '';
    }

    const previewEl = document.getElementById('metas-preview');
    const registros = await dbGetAll('registros');
    const now = new Date();
    const gastosPorCat = {};
    registros.forEach(r => {
        const [y, m] = r.data.split('-').map(Number);
        if (r.tipo === 'gasto' && y === now.getFullYear() && (m - 1) === now.getMonth()) {
            gastosPorCat[r.categoria] = (gastosPorCat[r.categoria] || 0) + r.valor;
        }
    });
    ...
    previewEl.innerHTML = metasAtivas.slice(0, 3).map(([cat, meta]) => {
        const info = getCategoriaInfo(cat);
        const g = gastosPorCat[cat] || 0;
        const pct = Math.min((g / meta) * 100, 100);
        const lv = pct >= 100 ? 'danger' : pct >= 75 ? 'warning' : 'safe';
        return `<div class="meta-preview-item">
            <span class="meta-preview-icon">${info.icon}</span>
            <div class="meta-preview-info"><div class="meta-preview-name">${info.label}</div><div class="meta-preview-bar"><div class="meta-preview-fill ${lv}" style="width:${pct}%"></div></div></div>
            <span class="meta-preview-valor">${formatMoney(g)}</span>
        </div>`;
    }).join('');
}

// ===== FIXAS =====
let filtroFixaTipo = 'todos';
let editingFixaId = null;
let currentFixaDetalheId = null;
...
function initFixas() {
    ...
    document.getElementById('modal-fixa-overlay').addEventListener('click', (e) => {
        if (e.target === e.currentTarget) closeFixaModal();
    });
    ...
    document.getElementById('modal-fixa-detalhe-overlay').addEventListener('click', (e) => {
        if (e.target === e.currentTarget) closeFixaDetalhe();
    });
    ...
    document.getElementById('btn-aplicar-fixas-dash')?.addEventListener('click', async () => {
        await aplicarTodasFixas();
...
function closeFixaModal() {
    document.getElementById('modal-fixa-overlay').classList.remove('open');
    editingFixaId = null;
}

function closeFixaDetalhe() {
    document.getElementById('modal-fixa-detalhe-overlay').classList.remove('open');
    currentFixaDetalheId = null;
}
...
async function salvarFixa() {
    ...
    const fixa = {
        id: editingFixaId || generateId(),
        tipo,
        descricao: desc,
        valor,
        dia: parseInt(document.getElementById('select-fixa-dia').value),
        ativa: true
    };

    if (tipo === 'gasto') {
        fixa.categoria = document.getElementById('select-fixa-categoria').value;
        fixa.metodo = document.getElementById('select-fixa-metodo').value;
    } else if (tipo === 'investimento') {
        fixa.tipoInvestimento = document.getElementById('select-fixa-tipo-inv').value;
    } else {
        fixa.metodo = document.getElementById('select-fixa-metodo').value;
    }
...
async function gerarRegistrosRecorrentes(fixa, meses) {
    if (!meses) meses = 12;
    const now = new Date();
    const registros = await dbGetAll('registros');
...
async function aplicarTodasFixas() {
    ...
    for (const fixa of fixas.filter(f => f.ativa !== false)) {
        await gerarRegistrosRecorrentes(fixa, 12);
    }
...
async function openFixaDetalhe(id) {
    ...
    if (isAp) {
        html += `<button class="btn-toggle-status btn-reverter" id="btn-toggle-fixa-status">↩️ Reverter (remover todos)</button>`;
    } else {
        html += `<button class="btn-toggle-status btn-aplicar-individual" id="btn-toggle-fixa-status">✅ Aplicar (12 meses)</button>`;
    }
    ...
    document.getElementById('btn-toggle-fixa-status').addEventListener('click', async () => {
        if (isAp) {
            ...
            showToast(`✅ "${fixa.descricao}" aplicada!`);
        }
        closeFixaDetalhe();
        await updateFixas();
        if (currentScreen === 'screen-dashboard') await updateDashboard();
...
async function excluirFixa() {
    ...
    const registros = await dbGetAll('registros');
    for (const r of registros) {
        if (r.fixaId === currentFixaDetalheId && r.fixaRecorrente) await dbDelete('registros', r.id);
    }
    await dbDelete('fixas', currentFixaDetalheId);
    closeFixaDetalhe();
    showToast('🗑️ Fixa excluída');
    updateFixas();
    if (currentScreen === 'screen-dashboard') await updateDashboard();
}
...
async function updateFixas() {
    ...
    if (filtered.length === 0) {
        ...
        listEl.innerHTML = filtered.map(f => {
            ...
            return `<div class="fixa-item" data-id="${f.id}">
                <div class="fixa-icon ${f.tipo}">${f.tipo === 'gasto' ? getCategoriaInfo(f.categoria).icon : f.tipo === 'investimento' ? getInvestimentoInfo(f.tipoInvestimento).icon : '💵'}</div>
                <div class="fixa-info"><div class="fixa-desc">${f.descricao}</div><div class="fixa-meta">Dia ${f.dia} · ${badge}</div></div>
                <div class="fixa-valor ${f.tipo}">${formatMoney(f.valor)}</div>
            </div>`;
        }).join('');
        listEl.querySelectorAll('.fixa-item').forEach(i => {
            i.addEventListener('click', () => openFixaDetalhe(i.dataset.id));
        });
    }
    ...
    document.getElementById('fixas-totais').innerHTML = `
        <div class="fixas-total-row"><span class="fixas-total-label">Receitas fixas</span><span class="fixas-total-valor receita">+ ${formatMoney(tR)}</span></div>
        <div class="fixas-total-row"><span class="fixas-total-label">Gastos fixos</span><span class="fixas-total-valor gasto">- ${formatMoney(tG)}</span></div>
        ${tI > 0 ? `<div class="fixas-total-row"><span class="fixas-total-label">Invest. fixos</span><span class="fixas-total-valor investimento">→ ${formatMoney(tI)}</span></div>` : ''}
        <div class="fixas-total-row"><span class="fixas-total-label"><strong>Saldo fixo</strong></span><span class="fixas-total-valor">${formatMoney(tR - tG - tI)}</span></div>`;
    ...
    if (pends.length > 0) {
        ...
        document.getElementById('fixas-pend-full-list').innerHTML = pends.map(f =>
            `<div class="fixa-pend-item" data-id="${f.id}">
                <span class="fixa-pend-icon">${f.tipo === 'gasto' ? getCategoriaInfo(f.categoria).icon : f.tipo === 'investimento' ? getInvestimentoInfo(f.tipoInvestimento).icon : '💵'}</span>
                <span class="fixa-pend-desc">${f.descricao}</span>
                <span class="fixa-pend-valor ${f.tipo}">${formatMoney(f.valor)}</span>
                <span class="fixa-pend-aplicar">Aplicar →</span>
            </div>`
        ).join('');
        document.getElementById('fixas-pend-full-list').querySelectorAll('.fixa-pend-item').forEach(el => {
            el.addEventListener('click', async () => {
                const fx = fixas.find(f => f.id === el.dataset.id);
                if (fx) {
                    await aplicarFixaIndividual(fx);
                    showToast(`✅ "${fx.descricao}" aplicada!`);
                    await updateFixas();
                }
            });
        });
...
async function updateFixasPendentes() {
    ...
    registros.forEach(r => {
        if (r.fixaId) {
            const [y, m] = r.data.split('-').map(Number);
            if (y === now.getFullYear() && (m - 1) === now.getMonth()) aplicadas.add(r.fixaId);
        }
    });
    ...
    document.getElementById('fixas-pendentes-list').innerHTML = pends.slice(0, 4).map(f =>
        `<div class="fixa-pend-item" data-id="${f.id}">
            <span class="fixa-pend-icon">${f.tipo === 'gasto' ? getCategoriaInfo(f.categoria).icon : f.tipo === 'investimento' ? getInvestimentoInfo(f.tipoInvestimento).icon : '💵'}</span>
            <span class="fixa-pend-desc">${f.descricao}</span>
            <span class="fixa-pend-valor ${f.tipo}">${formatMoney(f.valor)}</span>
...
// ===== GRÁFICOS =====
let graficoMes = new Date().getMonth();
let graficoAno = new Date().getFullYear();

function initGraficos() {
    document.getElementById('grafico-mes-prev').addEventListener('click', () => {
        graficoMes--;
        if (graficoMes < 0) { graficoMes = 11; graficoAno--; }
        updateGraficos();
    });
    document.getElementById('grafico-mes-next').addEventListener('click', () => {
        graficoMes++;
        if (graficoMes > 11) { graficoMes = 0; graficoAno++; }
        updateGraficos();
    });
}
...
async function updateGraficos() {
    ...
    const doMes = registros.filter(r => {
        const [y, m] = r.data.split('-').map(Number);
        return y === graficoAno && (m - 1) === graficoMes;
    });
    ...
    // Pizza
    const gastosPorCat = {};
    doMes.filter(r => r.tipo === 'gasto').forEach(r => {
        gastosPorCat[r.categoria] = (gastosPorCat[r.categoria] || 0) + r.valor;
    });
    ...
    if (catEntries.length === 0) {
        ...
        let svgSlices = '';
        slices.forEach(s => {
            svgSlices += `<circle cx="50" cy="50" r="40" fill="none" stroke="${s.color}" stroke-width="20" stroke-dasharray="${s.pct} ${100 - s.pct}" stroke-dashoffset="${25 - s.start}"/>`;
        });
    ...
    for (let i = 5; i >= 0; i--) {
        ...
        registros.forEach(r => {
            ...
            if (ry === yy && (rm - 1) === mm) {
                ...
                if (r.tipo === 'gasto') gas += r.valor;
                if (r.tipo === 'investimento') inv += r.valor;
            }
        });
    ...
    barrasContainer.innerHTML = meses6.map(m => {
        ...
        return `<div class="barra-grupo"><div class="barra-conjunto"><div class="barra receita" style="height:${hR}px"></div><div class="barra gasto" style="height:${hG}px"></div><div class="barra investimento" style="height:${hI}px"></div></div><span class="barra-label">${m.label}</span></div>`;
    }).join('');
...
async function updateGraficoPatrimonio(registros) {
    ...
    container.innerHTML = `<div class="patrimonio-chart">
        <svg class="patrimonio-chart-svg" viewBox="0 0 ${w} ${h}">
            <defs><linearGradient id="patrimonioGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="var(--green-medium)" stop-opacity="0.4"/><stop offset="100%" stop-color="var(--green-medium)" stop-opacity="0"/></linearGradient></defs>
            <path class="patrimonio-area" d="${areaPath}"/>
            <path class="patrimonio-line" d="${linePath}"/>
            ${dots}
        </svg>
    </div>
    <div class="patrimonio-labels">${pontos.map(p => `<div><div class="patrimonio-label-item">${p.label}</div><div class="patrimonio-valor-item">${formatMoney(p.valor)}</div></div>`).join('')}</div>`;
}
