...
const PinFeedback = (() => {
    ...
    function playClick(frequency = 1800, duration = 0.03, volume = 0.08) {
        try {
    ...
    return {
        tapKey() {
            vibrate(30);
            playClick(1800, 0.03, 0.08);
        },
        tapDelete() {
            vibrate(20);
            playClick(1200, 0.025, 0.05);
        },
        success() {
            vibrate([40, 60, 40]);
            playSuccess();
...
function openFixaModal(fixa) {
    ...
    if (fixa) {
        ...
        document.getElementById('input-fixa-desc').value = ''; document.getElementById('input-fi
